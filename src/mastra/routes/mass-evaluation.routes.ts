 ...
 */
export const massEvaluationStartRoute = registerApiRoute('/mass-evaluation/start', {
  method: 'POST',
  handler: async (c) => {
    try {
      ...
      const { taskId, batchId, fileUrl, questionDescription, userToken, callbackUrl } = body;

      // Validate required fields
      if (!taskId || !batchId || !fileUrl || !questionDescription || !callbackUrl) {
        return c.json({
          success: false,
          error: 'Missing required fields: taskId, batchId, fileUrl, questionDescription, callbackUrl',
        }, 400);
      }

      console.log(`[Mass Evaluation] Starting task ${taskId} for batch ${batchId}`);
      ...
      processEvaluationTask({
        ...
        console.error(`[Mass Evaluation] Error processing task ${taskId}:`, error);
      });
...
async function processEvaluationTask(params: {
  ...
  const { taskId, batchId, fileUrl, questionDescription, userToken, callbackUrl, mastra } = params;
  ...
  try {
    ...
    console.log(`[Task ${taskId}] Workflow completed with status: ${workflowOutput.status}`);
 ...
 */
export const massEvaluationStatsRoute = registerApiRoute('/mass-evaluation/stats', {
  method: 'GET',
  handler: async (c) => {
    try {
